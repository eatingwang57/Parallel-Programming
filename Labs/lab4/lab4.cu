...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    __shared__  char sharedR[5][128+4];
    __shared__  char sharedG[5][128+4];
    __shared__  char sharedB[5][128+4];
    for (int v = -yBound; v <= yBound; ++v) {
        if(bound_check(y + v, 0, height)) {
            sharedR[v+2][threadIdx.x+2] = s[channels * (width * (y + v) + (x)) + 2];
            sharedG[v+2][threadIdx.x+2] = s[channels * (width * (y + v) + (x)) + 1];
            sharedB[v+2][threadIdx.x+2] = s[channels * (width * (y + v) + (x)) + 0];
            if(threadIdx.x == 0 && x-2>=0) {
                sharedR[v+2][threadIdx.x+1] = s[channels * (width * (y + v) + (x-1)) + 2];
                sharedG[v+2][threadIdx.x+1] = s[channels * (width * (y + v) + (x-1)) + 1];
                sharedB[v+2][threadIdx.x+1] = s[channels * (width * (y + v) + (x-1)) + 0];
                sharedR[v+2][threadIdx.x] = s[channels * (width * (y + v) + (x-2)) + 2];
                sharedG[v+2][threadIdx.x] = s[channels * (width * (y + v) + (x-2)) + 1];
                sharedB[v+2][threadIdx.x] = s[channels * (width * (y + v) + (x-2)) + 0];
            }
            else if(threadIdx.x == blockDim.x -1 && x+2<width) {
                sharedR[v+2][threadIdx.x+3] = s[channels * (width * (y + v) + (x+1)) + 2];
                sharedG[v+2][threadIdx.x+3] = s[channels * (width * (y + v) + (x+1)) + 1];
                sharedB[v+2][threadIdx.x+3] = s[channels * (width * (y + v) + (x+1)) + 0];
                sharedR[v+2][threadIdx.x+4] = s[channels * (width * (y + v) + (x+2)) + 2];
                sharedG[v+2][threadIdx.x+4] = s[channels * (width * (y + v) + (x+2)) + 1];
                sharedB[v+2][threadIdx.x+4] = s[channels * (width * (y + v) + (x+2)) + 0];
            }
        }
    }
        ...
        for (int v = -yBound; v <= yBound; ++v) {
            for (int u = -xBound; u <= xBound; ++u) {
                if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                    const unsigned char R = sharedR[v+2][threadIdx.x+u+2];
                    const unsigned char G = sharedG[v+2][threadIdx.x+u+2];
                    const unsigned char B = sharedB[v+2][threadIdx.x+u+2];
...
int main(int argc, char **argv) {
    ...
    int maxNumThread = 128;
    int blockNum_x = width / maxNumThread + 1;
    int blockNum_y = height;
    dim3 block(blockNum_x, blockNum_y);
    dim3 thread(maxNumThread, 1);

    // launch cuda kernel
    sobel << <block, thread>>> (dsrc, ddst, height, width, channels);
