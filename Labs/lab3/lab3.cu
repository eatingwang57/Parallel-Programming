...
int main(int argc, char** argv) {
    ...
    int maxNumThread = 16; // 27
    int blockNum_x = width / maxNumThread + 1;
    int blockNum_y = height / maxNumThread + 1;
    dim3 block(blockNum_x, blockNum_y);
    dim3 thread(maxNumThread, maxNumThread);

    sobel<<< block, thread >>>(device_s, device_t, height, width, channels); 
